...
namespace Microsoft.Linq.Dynamic
{
    internal class ExpressionParser
    {
        ...
        public ParameterExpression IteratorParameter
        {
            get;
            private set;
        }
...
#pragma warning restore 0219
        ...
        Expression ParseIdentifier()
        {
            ...
            string qName = token.text;

            if ( identifierResolver != null && identifierResolver.HasValue( qName ) )
            {
                value = identifierResolver.GetValue( IteratorParameter, qName );
                return ParseResolverValue( value );
            }

            // check support for "full qualified" identifiers
            if ( identifierResolver != null && LookAhead() == '.' )
            {
                qName += ".";
                ...
                if ( identifierResolver.HasValue( qName ) )
                {
                    value = identifierResolver.GetValue( IteratorParameter, qName );
                    return ParseResolverValue( value );
                }
            ...
            else
            {
                ...
                {
                    return ParseMemberAccess( null, IteratorParameter );
                }
            }

            throw ParseError( Res.UnknownIdentifier, qName );
        }

        private Expression ParseResolverValue( object value )
        {
        ...
        Expression ParseLambdaInvocation( LambdaExpression lambda )
        {
            ...
            {
                // not a function call - resolver returned lazy function for identifier
        ...
        {
            ParameterExpression outerIt = IteratorParameter;
            ParameterExpression innerIt = Expression.Parameter( elementType, "" );
            IteratorParameter = innerIt;
            Expression[] args = ParseArgumentList();
            IteratorParameter = outerIt;
